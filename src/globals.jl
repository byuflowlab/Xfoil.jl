...
Wrap the global variables of the XFOIL shared library opened at `handle`
(a `Libdl.dlopen` handle) in an [`XfoilGlobals`](@ref) struct.
"""
get_globals

"""
    get_globals_cs(handle)
...
function get_globals(handle, ::Type{T}) where {T}

    cr01 = Ptr{T}(dlsym(handle, :cr01_))
    version = unsafe_wrap(Array,cr01,1)

    cr03 = Ptr{T}(dlsym(handle, :cr03_))
    aij = unsafe_wrap(Array,cr03,(IQX,IQX))
    dij = unsafe_wrap(Array,cr03+sizeof(T)*IQX^2,(IZX,IZX))

    cr04 = Ptr{T}(dlsym(handle, :cr04_))
    ...
    ci01 = Ptr{Int32}(dlsym(handle, :ci01_))
    ...
    ci03 = Ptr{Int32}(dlsym(handle, :ci03_))
    ...
    ci04 = Ptr{Int32}(dlsym(handle, :ci04_))
    ...
    ci05 = Ptr{Int32}(dlsym(handle, :ci05_))
    ...
    cl02 = Ptr{Int32}(dlsym(handle, :cl02_))
    tforce = unsafe_wrap(Array,cl02,ISX)

    cc01 = Ptr{UInt8}(dlsym(handle, :cc01_))
    ...
    cc02 = Ptr{UInt8}(dlsym(handle, :cc02_))
    labref = unsafe_string(cc02,32)

    cc03 = Ptr{UInt8}(dlsym(handle, :cc03_))
    vmxbl = unsafe_string(cc03,1)

    #var1
    var1 = Ptr{T}(dlsym(handle, :var1_))
...
get_globals(handle) = get_globals(handle, Float64)
get_globals_cs(handle) = get_globals(handle, ComplexF64)
