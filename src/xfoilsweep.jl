...
function xfoilsweep(x::AbstractArray{<:Real,1}, y::AbstractArray{<:Real,1},
    ...
    for i in eachindex(aoa)
        ...
        # run XFOIL
        cl[i], cd[i], cdp[i], cm[i], converged[i] = solveAlpha(aoa[i], re, mach=mach, iter=iter)
        ...
        if !converged[i] && percussive_maintenance
            cl[i], cd[i], cdp[i], cm[i], converged[i] =
                dopercussivemaintainance(x, y, aoa[i], re, mach, iter, npan)
        end
...
function dopercussivemaintainance(x::AbstractArray{<:Real,1},y::AbstractArray{<:Real,1},
    ...
    for j = 1:25
        ...
        setCoordinates(x,y)
        pane(npan=npan)
        cl,cd,cdp,cm,converged = solveAlpha(aoamod, remod, mach=mach, iter=iter)
        if converged
            break
        end
    end
    ...
    if converged
        ...
        cl,cd,cdp,cm,converged = solveAlpha(aoa,re,mach=mach,iter=iter)
    end
    return cl, cd, cdp, cm, converged
end
