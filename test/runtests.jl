...
# solveAlpha test
cl, cdaf, cdp, cm, conv = Xfoil.solveAlpha(5.0, 1e5, iter=100)
...
cls, cds, cdps, cms, convs =Xfoil.xfoilsweep(x, y, aoas, 1e5; iter=100,
...
Xfoil.setCoordinates_cs(x .+ 1e-20im, y .+ 1e-20im)
...
# solveAlpha_cs test
cl_cs, cd_cs, cdp_cs, cm_cs, conv_cs = Xfoil.solveAlpha_cs(5.0+1e-20im, 1e5,
    iter=100)
...
# xfoilsweep_cs test
aoas_cs = convert(Array{ComplexF64,1}, aoas).+1e-30im
cls_cs, cds_cs, cdps_cs, cms_cs, convs_cs =Xfoil.xfoilsweep_cs(x, y, aoas_cs, 1e5;
